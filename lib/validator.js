    ...
    switch (field.datatype) {
      case 'DATE':
        if (requestBody[field.name + '-year'] && requestBody[field.name + '-month'] && requestBody[field.name + '-day']) {
          formBody[field.name] = `${requestBody[field.name + '-year'].padStart(2, '0')}-${requestBody[field.name + '-month'].padStart(2, '0')}-${requestBody[field.name + '-day']}`
        } else {
          formBody[field.name] = null
        }
        break
      default:
        formBody[field.name] = requestBody[field.name]
    }
  }
