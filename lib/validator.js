...
const sanatize = (dataset, requestBody) => {
  ...
  for (const field of dataset.fields) {
    switch (field.datatype) {
      case 'DATE':
        if (requestBody.hasOwnProperty(field.name + '-year') || requestBody.hasOwnProperty(field.name + '-month') || requestBody.hasOwnProperty(field.name + '-day')) {
          const year = requestBody[field.name + '-year']
          const month = requestBody[field.name + '-month']
          const day = requestBody[field.name + '-day']
          if (year || month || day) {
            formBody[field.name] = `${year.padStart(4, '0')}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`
          } else {
            formBody[field.name] = null
          }
          delete formBody[field.name + '-year']
          delete formBody[field.name + '-month']
          delete formBody[field.name + '-day']
        } else {
